...
이러한 문제를 해결하기 위해 SSR이 다시 주목받기 시작했다. 하지만 SSR은 트레이드오프가 분명했다. 빠른 초기 페이지 로드, SEO 측면에 장점이 있었다. 또 클라이언트 자원 사용을 낮출 수 있다. 하지만 역으로 서버 자원이 필요하다. 위에서 React 라이브러릴 쓰더라도 SSR은 여전히 구현이 복잡하고, 상태 관리의 어려움도 있었다.
